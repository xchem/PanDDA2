...
def get_event_info(dataset_dir):
    ...
    return event_info, best_event_id
...
def main():
    ...
    ligand_files = parse_dir_ligands(dataset_dir / 'ligand_data')
    ...
    event_map_grid = read_pandda_map(
        dataset_dir / constants.PANDDA_EVENT_MAP_FILE.format(
            ...
            bdc=event_info[best_event_id]['bdc'],
            )
            )
    ...
    raw_xmap_grid = read_mtz(dataset_dir / constants.PANDDA_MTZ_FILE.format(dtag=dtag))
    ...
    centroid = event_info[best_event_id]['bdc']['centroid']
    rprint(centroid)

    # Build
    rprint('# Generating builds...')
    builds, scores = {}, {}
    for ligand_key, conformer in conformers.items():
        ...
        builds[ligand_key] = optimized_structure
        scores[ligand_key] = score
    rprint(builds)
    rprint(scores)
